...
def doc_2_vec(content, label, vector, df_below, df_above, length):
    len_sms = []
    list_corpus = []
    list_corpus_new = []
    for line in content:
    ...
    bigram = models.Phrases(list_corpus, min_count=10)
    list_corpus = bigram[list_corpus]
    ...
    #dictionary_bigram = corpora.Dictionary(list_corpus)
...
def count_vocab(list_content):
    #dictionary = corpora.Dictionary(list_content)
    #print dictionary
    list_vocab = []
    list_corpus = []
    count = 1
...
if __name__ == "__main__":
    corpus_name = 'corpus_train.txt'
    list_label, list_content = load_corpus(corpus_name)
    #count_vocab(list_content)
    list_content_vec, list_label, list_len_sms, dictionary = doc_2_vec(list_content, list_label, 'bow', 3, 1.0, 1)
    print np.shape(list_content_vec)
    list_matrix, list_false_positive, list_false_negative, list_true_positive, list_true_negative = \
        k_fold(list_content, list_content_vec, list_label, 'svm')
    evaluation(list_false_positive, list_false_negative, list_true_positive, list_true_negative)